...
- **browser_type**: `chrome` (the default) or `firefox`. I initially wrote this code for Firefox, but Chrome seems to handle the Cloudflare verification better; it rarely runs into a blocking loop.
...
Either [Chrome] or [Firefox] are required. Chrome is the default since it seems
to handle the Cloudflare CAPTCHA better. You need to install whatever browser
...
1. Consider using Chrome instead of Firefox. For unexplained reasons it seems 
   to be more robust.
2. Contact the staff at the Data Rescue Project and ask them to have your
   machine added to the allowlist. Give them your external IP address,
...
This repository is based on an old one at
https://github.com/YNodd/create-DataLumos-projects-from-csv-file